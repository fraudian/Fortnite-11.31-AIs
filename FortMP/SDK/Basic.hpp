...
public:
	...
	void Reserve(int Number, int Size = sizeof(T))
	{
		Data = (T*)FMemory_Realloc(Data, (MaxElements = NumElements + Number) * Size, 0);
	}

	void FreeArray()
	{
		if (Data && FMemory_Free)
		{
			FMemory_Free(Data);
		}
	...
	int Add(const T& New, int Size = sizeof(T))
	{
		Reserve(1, Size);

		if (Data)
		{
			Data[NumElements] = New;
			++NumElements;
			++MaxElements;
			return NumElements;
		}
